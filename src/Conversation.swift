...
public final class Conversation: Sendable {
  ...
  @MainActor public func whenConnected<E>(_ callback: @Sendable () async throws(E) -> Void)
    async throws(E)
  {
  ...
  public func send(from role: Item.ItemRole, text: String, response: Response.Config? = nil)
    async throws
  {
      ...
      event: .createConversationItem(
        Item(
          message: Item.Message(
            id: String(randomLength: 32), from: role, content: [.input_text(text)]))))
...
extension Conversation {
  ...
  @MainActor public func startListening(
    ...
    Task.detached {
      self.audioEngine.inputNode.installTap(
        onBus: 0, bufferSize: 4096, format: self.audioEngine.inputNode.outputFormat(forBus: 0)
  ...
  @MainActor public func startHandlingVoice(
    audioSession: AVAudioSession! = AVAudioSession.sharedInstance()
  ) throws {
    ...
    guard
      let converter = AVAudioConverter(
        from: audioEngine.inputNode.outputFormat(forBus: 0), to: desiredFormat)
    else {
  ...
  @MainActor public func interruptSpeech() {
    if isPlaying,
      let nodeTime = playerNode.lastRenderTime,
      let playerTime = playerNode.playerTime(forNodeTime: nodeTime),
      let itemID = queuedSamples.first
    {
      let audioTimeInMiliseconds = Int(
        (Double(playerTime.sampleTime) / playerTime.sampleRate) * 1000)

      Task {
        do {
          try await client.send(
            event: .truncateConversationItem(forItem: itemID, atAudioMs: audioTimeInMiliseconds))
        } catch {
...
extension Conversation {
  @MainActor fileprivate func handleEvent(_ event: ServerEvent) {
    switch event {
    ...
    case let .conversationItemInputAudioTranscriptionCompleted(event):
      updateEvent(id: event.itemId) { message in
        ...
        message.content[event.contentIndex] = .input_audio(
          .init(audio: audio.audio, transcript: event.transcript))
      }
    ...
    case let .responseAudioTranscriptDelta(event):
      updateEvent(id: event.itemId) { message in
        ...
        message.content[event.contentIndex] = .audio(
          .init(audio: audio.audio, transcript: (audio.transcript ?? "") + event.delta))
      }
    ...
    case let .responseAudioDelta(event):
      updateEvent(id: event.itemId) { message in
        ...
        message.content[event.contentIndex] = .audio(
          .init(audio: audio.audio + event.delta, transcript: audio.transcript))
      }
  ...
  @MainActor
  fileprivate func updateEvent(id: String, modifying closure: (inout Item.Message) -> Void) {
  ...
  @MainActor
  fileprivate func updateEvent(id: String, modifying closure: (inout Item.FunctionCall) -> Void) {
    guard let index = entries.firstIndex(where: { $0.id == id }),
...
extension Conversation {
  private func queueAudioSample(_ event: ServerEvent.ResponseAudioDeltaEvent) {
    ...
    guard
      let converter = apiConverter.lazy({
        AVAudioConverter(from: buffer.format, to: playerNode.outputFormat(forBus: 0))
      })
    else {
    ...
    let outputFrameCapacity = AVAudioFrameCount(
      ceil(converter.outputFormat.sampleRate / buffer.format.sampleRate)
        * Double(buffer.frameLength))

    guard
      let sample = convertBuffer(buffer: buffer, using: converter, capacity: outputFrameCapacity)
    else {
  ...
  private func processAudioBufferFromUser(buffer: AVAudioPCMBuffer) {
    ...
    guard
      let convertedBuffer = convertBuffer(
        buffer: buffer, using: userConverter.get()!,
        capacity: AVAudioFrameCount(Double(buffer.frameLength) * ratio))
    else {
    ...
    let audioData = Data(
      bytes: sampleBytes, count: Int(convertedBuffer.audioBufferList.pointee.mBuffers.mDataByteSize)
    )
  ...
  private func convertBuffer(
    buffer: AVAudioPCMBuffer, using converter: AVAudioConverter, capacity: AVAudioFrameCount
  ) -> AVAudioPCMBuffer? {
